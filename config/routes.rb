def setup_response(response, access_token)
...
token_endpoint = Rack::OAuth2::Server::Token.new do |req, res|
  ...
  when :authorization_code
    ...
    setup_response res, code.access_token
  when :password
    ...
    setup_response res, account.access_tokens.create(:client => client)
  when :client_credentials
    # NOTE: client is already authenticated here.
    setup_response res, client.access_tokens.create
  else
    # NOTE: refresh_token and other assertions are not supported yet.
